// In production (Docker), use relative URLs which go through nginx proxy
...
export const API_ENDPOINTS = {
  ...
  userProfileById: (profileId: number) =>
    `${API_BASE_URL}/api/v1/profiles/${profileId}`,
  ...
  // Skills
  userSkills: (userId: number) =>
    `${API_BASE_URL}/api/v1/users/${userId}/skills`,
  skillsById: (id: number) => `${API_BASE_URL}/api/v1/skills/${id}`,

  // CVs
  projectCVs: (projectId: number) =>
    `${API_BASE_URL}/api/v1/projects/${projectId}/cvs`,
  cvById: (cvId: number) => `${API_BASE_URL}/api/v1/cvs/${cvId}`,
  cvRegenerate: (cvId: number) =>
    `${API_BASE_URL}/api/v1/cvs/${cvId}/regenerate`,
  ...
  userApplications: (userId: number) =>
    `${API_BASE_URL}/api/v1/users/${userId}/applications`,

  // Extraction
  extractProfile: (userId: number) =>
    `${API_BASE_URL}/api/v1/users/${userId}/extract-profile`,
};
